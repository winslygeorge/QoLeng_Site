...
export function undoState() {
  if (stateHistory.length > 1) {
    ...
    window._clientVars.currentState = { ...state };
    notifySubscribers();
  }
}
...
export function initializeStore(initialState = {}) {
  window._clientVars.currentState = { ...initialState };
  window._clientVars.previousState = { ...initialState };
  notifySubscribers();
  persistStateSlices();
}
...
function applyMiddleware(action) {
  for (const fn of middlewares) {
    const result = fn(action, window._clientVars.currentState, dispatch, getState);
...
function logAction(action, oldState, newState) {
  if (devToolsEnabled) {
    ...
    console.debug("Prev:", oldState);
    console.debug("Next:", newState);
    console.groupEnd();
  }
  if (trackHistory) {
...
function notifySubscribers() {
  for (const cb of subscribers) {
    try {
      cb({ ...window._clientVars.currentState });
    } catch (e) {
...
export function getState() {
  return { ...window._clientVars.currentState };
}
...
export async function dispatch(action) {
  ...
  if (typeof reducer === "function") {
    const result = reducer(window._clientVars.currentState, action);
    const newState = result instanceof Promise ? await result : result;

    if (newState !== window._clientVars.currentState) {
      logAction(action, window._clientVars.currentState, newState);
      window._clientVars.currentState = newState;
      notifySubscribers();
      persistStateSlices();
    }
  } else {
...
export function hydrateStore(serverState) {
  window._clientVars.currentState = { ...window._clientVars.currentState, ...serverState };
  window._clientVars.previousState = { ...window._clientVars.currentState };
  notifySubscribers();
}
...
export function enablePersistenceSlice(key, { ttlSeconds = null, compressed = false } = {}) {
  ...
  try {
    ...
    if (json.hasOwnProperty('value')) {
      window._clientVars.currentState = { ...window._clientVars.currentState, [key]: json.value };
    } else {
      delete json.__timestamp;
      window._clientVars.currentState = { ...window._clientVars.currentState, [key]: json };
    }
...
function persistStateSlices() {
  const changed = jsonDiff(window._clientVars.previousState, window._clientVars.currentState);
  for (const key in persistSlices) {
    ...
    let serialized;
    try {
      const valueToStore = window._clientVars.currentState[key];
...
export function bindUIToStore() {
  ...
  updateBindings(window._clientVars.currentState);
}
