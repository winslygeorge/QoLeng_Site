...
function RedisBackendStrategy:call_pipeline(commands)
    local results = {}

    for i, cmd in ipairs(commands) do
        local ok, res = self:command(unpack(cmd))
        if not ok then
            return false, string.format(
                "Pipeline command failed at index %d: %s",
                i, res
            )
        end
...
function RedisBackendStrategy:command(cmd, ...)
    if not self.redis then
        return false, "Redis client not initialized"
    end

    local method = cmd:lower()  -- 🔥 THIS FIXES YOUR ERROR
    local f = self.redis[method]

    if type(f) ~= "function" then
        return false, "Redis command not supported: " .. cmd
    end

    local ok, res = pcall(f, self.redis, ...)
    if not ok then
        return false, res
    end
